...
# Upewnij się, że DB_URL jest w Secrets na Streamlit Cloud!
try:
    DB_URL = st.secrets["DB_URL"]
    engine = sqlalchemy.create_engine(DB_URL)
except:
    st.error("Błąd: Brak konfiguracji DB_URL w Secrets!")
...
def get_vda_ap(s, o, d):
    # Prosta logika AP (możesz ją rozbudować o pełną matrycę VDA)
    score = s * o * d
...
st.markdown("""
    ...
    .stButton > button { height: 38px !important; border-radius: 4px !important; background-color: #1f2937 !important; color: #e5e7eb !important; width: 100%; }
    div[data-testid="column"] button { height: 28px !important; min-height: 28px !important; padding: 0px !important; font-size: 14px !important; }
...
# --- PDF (BEZPIECZNY DLA LINUXA) ---
class FMEA_PDF(FPDF):
    def __init__(self, user_name, **kwargs):
        super().__init__(**kwargs)
        self.user_name = user_name
        self.pl_font = 'Helvetica' # Standardowa bezpieczna czcionka
...
def generate_pdf(df, project_name, user_name):
    ...
    for _, row in df.iterrows():
        ...
        pdf.cell(w[8], 8, ap_val, border=1, align='C', fill=True)
...
def modal_wpis(op_id, op_nazwa):
    ...
    with st.form("fm_new"):
        ...
        pre = st.text_area("Prewencja", height=68)
        ...
        if show_actions:
            dz = st.text_input("Działania")
            kt = st.text_input("Kto")
...
def modal_edycja_wpisu(row):
    with st.form("fm_edit"):
        wada = st.text_input("Błąd", value=row['wada'])
        skutek = st.text_input("Skutek", value=row['skutek'])
        s = st.select_slider("S ", options=range(1, 11), value=int(row['s']))
        o = st.select_slider("O ", options=range(1, 11), value=int(row['o']))
        d = st.select_slider("D ", options=range(1, 11), value=int(row['d']))
        dz = st.text_input("Działania", value=row['dzialanie'])
        kt = st.text_input("Kto", value=row['kto'])
...
if sel_p == "DASHBOARD":
    ...
    if st.session_state.role == "admin":
        ...
        with t1:
            ...
            np = c1.text_input("Nowy projekt", placeholder="Nazwa...")
            if c2.button("UTWÓRZ") and np:
                run_query("INSERT INTO projekty (nazwa) VALUES (:n)", {"n": np}); st.rerun()
            for _, rp in p_df.iterrows():
                r = st.columns([4,1])
                r[0].write(f"📂 **{rp['nazwa']}**")
                if r[1].button("USUŃ", key=f"delp_{rp['id']}"):
        ...
        with t2:
            ...
            for _, ru in u_list.iterrows():
                if ru['username'] != 'admin':
                    ...
                    if r[1].button("📝", key=f"u_{ru['username']}"): modal_edit_user(ru)
                    if r[2].button("✖", key=f"d_{ru['username']}"):
...
# --- WIDOK PROJEKTU ---
    ...
    df_all = get_data("""SELECT w.*, o.nazwa FROM wpisy w 
                         JOIN operacje o ON w.op_id = o.id 
                         WHERE o.p_id = :p_id""", {"p_id": int(p_id)})
    
    if not df_all.empty:
        pdf_b = generate_pdf(df_all, sel_p, st.session_state.user)
        ch2.download_button("📄 PDF", data=pdf_b, file_name=f"FMEA_{sel_p}.pdf")

    if st.session_state.role == "admin":
        with st.container(border=True):
            c1, c2 = st.columns([4, 1])
            new_op = c1.text_input("Nowy krok procesu")
            if c2.button("✚ KROK") and new_op:
    ...
    for _, op in ops.iterrows():
        with st.expander(f"⚙️ {op['nazwa']}", expanded=True):
            c_size = [1.2, 1.2, 0.4, 1.2, 1.2, 0.4, 1.2, 0.4, 0.5, 2.0, 1.2, 1.0, 0.6, 0.6]
            if st.session_state.role == "admin":
                if st.button("✚ DODAJ WPIS", key=f"add_{op['id']}"): modal_wpis(op['id'], op['nazwa'])
            
            df_w = df_all[df_all['op_id'] == op['id']]
            for _, row in df_w.iterrows():
                r = st.columns(c_size)
                r[0].write(row['wada']); r[2].write(f"**{row['s']}**"); r[5].write(str(row['o'])); r[7].write(str(row['d']))
                clr = "#ef4444" if row['ap'] == "H" else "#f59e0b" if row['ap'] == "M" else "#10b981"
                r[8].markdown(f"<p style='color:{clr};font-weight:bold;'>{row['ap']}</p>", unsafe_allow_html=True)
                r[9].write(row['dzialanie'])
                if st.session_state.role == "admin":
